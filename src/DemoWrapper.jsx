...
export default function DemoWrapper() {
  ...
  if (isAdminMode) {
    return <App onAdminChange={setIsAdminMode} />;
  }

  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f5f5f5", fontFamily: "'Figtree', system-ui, sans-serif", color: "#1a1a22" }}>
      ...
      <aside style={{ width: 320, flexShrink: 0, background: "#ffffff", borderRight: "1px solid #e5e7eb", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 10, overflowY: "auto" }}>
        ...
        <div style={{ padding: "20px 24px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: accent, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Darker Grotesque', serif", fontSize: 22, fontWeight: 700, color: "#fff" }}>
              {c.studio.logo}
            </div>
            <div>
              <div style={{ fontFamily: "'Darker Grotesque', serif", fontSize: 18, fontWeight: 600, letterSpacing: "0.02em", color: "#1a1a22" }}>{c.studio.name}</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {c.features.map((f, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 10px", borderRadius: 8, background: "#f9fafb" }}>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#1a1a22" }}>{f.label}</div>
      ...
      <main style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "flex-start", paddingTop: 32, paddingBottom: 32, marginLeft: 320, marginRight: 340 }}>
        <div style={{ position: "relative" }}>
          ...
          <div style={{
            ...
            boxShadow: "0 0 0 1px rgba(0,0,0,.08), 0 20px 60px rgba(0,0,0,.12), 0 8px 24px rgba(0,0,0,.08)"
          }}>
            {/* Notch */}
      ...
      <aside style={{ width: 340, flexShrink: 0, background: "#ffffff", borderLeft: "1px solid #e5e7eb", position: "fixed", top: 0, right: 0, bottom: 0, overflowY: "auto", padding: "24px 20px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {c.salesCards.map((card, i) => {
            ...
            return (
              <div key={i} style={{ background: "#ffffff", border: "1px solid #e5e7eb", borderRadius: 14, padding: "18px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <h3 style={{ fontFamily: "'Darker Grotesque', serif", fontSize: 18, fontWeight: 600, color: "#1a1a22", margin: 0 }}>{card.title}</h3>
                </div>
                ...
                {isAdminCard && (
                  <button onClick={() => setIsAdminMode(true)} style={{
                    ...
                    border: "none", background: accent, color: "#fff", fontWeight: 700,
          ...
          {/* CTA Card */}
          <div style={{ background: `linear-gradient(135deg, ${accent}10, ${accentDark}06)`, border: `1px solid ${accent}25`, borderRadius: 14, padding: "22px 18px", textAlign: "center" }}>
            <h3 style={{ fontFamily: "'Darker Grotesque', serif", fontSize: 22, fontWeight: 600, color: "#1a1a22", margin: "0 0 6px" }}>{c.cta.heading}</h3>
